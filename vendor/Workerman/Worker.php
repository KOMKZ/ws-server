...
class Worker
{
    ...
    public static function runAll()
    {
        ...
        self::init();
        self::parseCommand();
        self::daemonize();// 守护进程化，pcntl_fork
        self::initWorkers(); // 对所有的worker实例 stream_socket_server
        self::installSignal(); // 对主进程注册信号， stop, reload, status, todo还没详细看
        self::saveMasterPid(); // 保存主进程的pid，供自杀用
        self::forkWorkers();
        self::displayUI();
        self::resetStd();
        self::monitorWorkers();
    ...
    protected static function init()
    {
        ...
        // /var/www/html/workerman-chat/start.php
        self::$_startFile = $backtrace[count($backtrace) - 1]['file'];

        // Pid file.
        if (empty(self::$pidFile)) {
            // /var/www/html/workerman-chat/Workerman/../_var_www_html_workerman-chat_start.php.pid
    ...
    protected static function initWorkers()
    {
        foreach (self::$_workers as $worker) {
            ...
            // 所谓的socket-name 就是 协议 + ip + 端口号那串东西
            ...
            // Listen.
            // 创建实例所对应的socket，用的是stream_socket流
            if (!$worker->reusePort) {
                $worker->listen();
            }
        }
    ...
    protected static function parseCommand()
    {
        ...
        switch ($command) {
            case 'kill':
                // http://zb00916.blog.51cto.com/3161557/959507
                // 信号量的说明可以参考这篇文章
                exec("ps aux | grep $start_file | grep -v grep | awk '{print $2}' |xargs kill -SIGINT");// 中断
                usleep(100000);
                exec("ps aux | grep $start_file | grep -v grep | awk '{print $2}' |xargs kill -SIGKILL");// 尽快终止，杀死
                break;
            case 'start':
                if ($command2 === '-d') {
    ...
    protected static function forkWorkers()
    {
        foreach (self::$_workers as $worker) {
            ...
            // [00000000048d1ef200000000157cde98] => array()
            // [00000000048d1ef000000000157cde98] => array()
            // [00000000048d1ef600000000157cde98] => array()
            // [00000000048d1ef400000000157cde98] => array()
            // 这个id就是 __construct 的 spl_object_hash($this)
            // 注意此时每个 worker 的socket已经是建立的
    ...
    {
        // Save all worker instances.  000000000fa44d930000000032bef1ba
    ...
    public function listen()
    {
        ...
        // 这里就是在 查找协议类了，如果要让自己的协议类能够识别的话，应该
        // 要对实例显式的注册 _autoloadRootPath 这样子就能够调用到了
        ...
        // local_socket : "tcp://0.0.0.0:7272", 7272 是监听的端口, 0.0.0.0即监听所有的网卡
    ...
    {
        // Accept a connection on server socket.等待连接的时候阻塞
