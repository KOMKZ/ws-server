<?php
namespace cchat\server;

use Yii;
use yii\helpers\ArrayHelper;
use yii\base\Component;
use cchat\exceptions\NotFoundCommandException;
use \GatewayWorker\BusinessWorker;
use \GatewayWorker\Gateway as GatewayWorker;
use \GatewayWorker\Lib\Gateway;
use \GatewayWorker\Register;
use yii\base\InvalidRouteException;
use cchat\exceptions\ChatRunException;
use cchat\base\Res;
 ...
 *
 */
class CChat extends Component implements BootstrapInterface
{
    private $_commandNameSpace = 'cchat\\server\\commands';
    ...
    private $defaultWorkerConfig = [
        'workerCount' => 4,
        'name' => 'BusinessWorker',
        'eventHandler' => '\\cchat\\server\\CChat',
        ...
        if(is_array($this->remoteConfigCallback) &&
           ...
           method_exists($this->remoteConfigCallback[0], $this->remoteConfigCallback[1])){
               ...
               foreach($config as $configName => $value){
                   Yii::$app->cchat->$configName = $value;
               }
        }else{
    ...
    public static function initBussinessWorker(){
        ...
        $worker->name = Yii::$app->cchat->workerConfig['name'];
        $worker->count = Yii::$app->cchat->workerConfig['workerCount'];
        $worker->eventHandler = Yii::$app->cchat->workerConfig['eventHandler'];
        $worker->registerAddress = Yii::$app->cchat->workerConfig['registerAddress'];
    }
    /**
     * 初始化路由worker
     * @return [type] [description]
    ...
    public static function initGatewayWorker(){
        $port = Yii::$app->cchat->gatewayConfig['port'];
        $gateway = new GatewayWorker("Websocket://0.0.0.0:{$port}");
        $gateway->name = Yii::$app->cchat->gatewayConfig['name'];
        $gateway->count = Yii::$app->cchat->gatewayConfig['count'];
        $gateway->lanIp = Yii::$app->cchat->gatewayConfig['lanIp'];
        $gateway->startPort = Yii::$app->cchat->gatewayConfig['startPort'];
        $gateway->pingInterval = Yii::$app->cchat->gatewayConfig['pingInterval'];
        $gateway->pingData = Yii::$app->cchat->gatewayConfig['pingData'];
        $gateway->registerAddress = Yii::$app->cchat->gatewayConfig['registerAddress'];
    }
    /**
     * 初始化中转worker
     * @return [type] [description]
    ...
    public static function initRegisterWorker(){
        $address = Yii::$app->cchat->registerConfig['address'];
    ...
    public static function onMessage($clientId, $data){
        Yii::$app->cchat->chatRun(['clientId' => $clientId, 'body' => $data]);
    }
    public static function onConnect($clientId){
        printf("new connect:%s\n", $clientId);
        Yii::$app->req->clientId = $clientId;
        if(Yii::$app->cchat->closeNotLogin){
            $maxFailedTime = Yii::$app->cchat->getNotLoginTime($_SERVER['REMOTE_ADDR']);
            if($maxFailedTime >= Yii::$app->cchat->maxFailedLoginTime){
                ...
                Yii::$app->cchat->incrementNotLogin($_SERVER['REMOTE_ADDR']);
            }
        }
    }
    ...
    public static function addAuthTimer($clientId){
        $limitTime = Yii::$app->cchat->maxDelayInLogin;
        $ifClientNoLogin = Yii::$app->cchat->ifClientNoLogin;
    ...
    public static function warning($msg){
        Yii::warning($msg, Yii::$app->cchat->name);
    }
    public static function error($msg){
        Yii::error($msg, Yii::$app->cchat->name);
    }
